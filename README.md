...
![训练曲线](results/mappo/3m/training_curve_0.png)
...
#### 训练过程观察
1. **前期探索阶段** (0-10000步): 胜率较低，智能体在学习基础动作
2. **快速提升阶段** (10000-25000步): 胜率快速上升，开始学会基础配合
3. **精细调优阶段** (25000步+): 胜率稳定，策略进一步优化

### 损失函数分析
从训练日志可以看到：
- **Policy Loss**: 逐渐收敛，表明策略更新稳定
- **Value Loss**: 前期较高后期稳定，价值函数学习正常
- **Entropy**: 保持适中水平，确保探索与利用平衡
...
### 训练建议
1. **从简单地图开始** - 先在3m等简单地图上验证算法
2. **渐进式训练** - 逐步增加训练步数和地图难度
3. **监控训练过程** - 注意胜率和损失函数的变化趋势

### 常见问题
1. **训练不收敛**: 尝试降低学习率或增加训练时间
2. **显存不足**: 减少批次大小或禁用经验回放
3. **环境配置**: 确保StarCraft II路径正确配置
...
如果有任何问题或建议，欢迎通过GitHub Issue联系我。